document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const errorMessages = {
      ...
      tooLong: (max) => `Please enter no more than ${max} characters.`,
    ...
    function getErrorMessage(field) {
      ...
      if (validity.valueMissing) {
        return field.dataset.errorRequired || errorMessages.valueMissing;
      }
      if (validity.typeMismatch) {
        return (
          field.dataset.errorType ||
          (errorMessages.typeMismatch[field.type]) ||
          errorMessages.default
        );
      }
      if (validity.tooShort) {
        return field.dataset.errorShort || errorMessages.tooShort(field.minLength);
      }
      if (validity.tooLong) {
        return field.dataset.errorLong || errorMessages.tooLong(field.maxLength);
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .catch(function (err) {
          ...
          if (!serverErrorEl) {
            serverErrorEl = document.createElement('p');
            serverErrorEl.className = 'form-server-error';
            serverErrorEl.setAttribute('role', 'alert');
            contactForm.appendChild(serverErrorEl);
          }
          serverErrorEl.textContent = contactForm.dataset.errorMessage || 'Something went wrong. Please try again later.';
        })
        .finally(function () {
          if (submitBtn) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '457f5f2d-3bb0-4d7f-8b05-cee80ee9c75e',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '457f5f2d-3bb0-4d7f-8b05-cee80ee9c75e';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '457f5f2d-3bb0-4d7f-8b05-cee80ee9c75e';
